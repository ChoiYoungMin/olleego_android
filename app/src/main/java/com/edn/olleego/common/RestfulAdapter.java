...
public class RestfulAdapter {
    ...
    private static OlleegoInterface olleegoInterface;

    public synchronized static OlleegoInterface getInstance() {
        if (olleegoInterface == null) {
            //통신로그를 확인하기 위한 부분
            ...
            //Retrofit 설정
            olleegoInterface = new Retrofit.Builder()
                    .baseUrl(OLLEEGO_HOST)
                    .client(client)
                    .addConverterFactory(GsonConverterFactory.create())
                    .build().create(OlleegoInterface.class); //인터페이스 연결
        }
        return olleegoInterface;
    }

    public static void setOlleegoInterface(OlleegoInterface olleegoInterface) {
        RestfulAdapter.olleegoInterface = olleegoInterface;
    }
